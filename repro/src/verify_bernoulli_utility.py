...
def allocation_epsilon(sigma: float, bound_type: BoundType) -> float:
...
def poisson_epsilon(sigma: float, pessimistic: bool) -> float:
...
def invert_decreasing(
    ...
    *,
    ...
    return {
        ...
        "epsilon_at_lower": function(left),
        "epsilon_at_upper": function(right),
        ...
        out / "claim_contract.json",
        {
            ...
            "quantifiers": {
                ...
                "required_relationship": (
                    "allocation upper-bound sigma < Poisson optimistic sigma, "
                    "and allocation analytic MSE < Poisson analytic MSE"
                ),
            },
            "verdict_rule": (
                "VERIFIED iff the robust noise ordering holds in every panel, "
...
For every privacy panel, required Gaussian noise is found by deterministic
bisection. Random-allocation epsilon comes from the pinned released author API;
both stochastic upper and lower PLD bounds are evaluated. Poisson epsilon is
computed independently with Google's `dp-accounting` PLD accountant; both
pessimistic and optimistic discretizations are evaluated. The strict comparison
uses the conservative allocation requirement against the optimistic Poisson
requirement, so numerical uncertainty cannot manufacture the reported ordering.
...
""",
    ...
    write_json(
        ...
        {
            "checker": "closed-form variance decomposition plus seeded simulation",
            "rows": utility_rows,
        },
    )
    write_json(
        ...
        {
            "mutation": (
                "replace the allocation privacy requirement with the Poisson "
                "requirement; strict lower-noise relation must disappear"
            ),
...
scale. New Monte Carlo draws use fixed disclosed seeds and are checked against
the exact moments rather than expected to match historical random draws.
""",
    )
    write_text(
        out / "EVAL.md",
...
def main() -> int:
    ...
    for panel, (epsilon, dimension) in enumerate(SETTINGS):
        allocation_upper = invert_decreasing(
            lambda sigma: allocation_epsilon(sigma, BoundType.DOMINATES),
            epsilon,
        )
        allocation_lower = invert_decreasing(
            lambda sigma: allocation_epsilon(sigma, BoundType.IS_DOMINATED),
            epsilon,
        )
        poisson_upper = invert_decreasing(
            lambda sigma: poisson_epsilon(sigma, True), epsilon
        )
        poisson_lower = invert_decreasing(
            lambda sigma: poisson_epsilon(sigma, False), epsilon
        )
        allocation_sigma = allocation_upper["sigma_upper_bracket"]
        poisson_sigma = poisson_upper["sigma_upper_bracket"]
        robust_lower_noise = (
            allocation_upper["sigma_upper_bracket"]
            < poisson_lower["sigma_lower_bracket"]
        )
        privacy_rows.append(
            {
                "panel": panel,
                ...
                "t": T,
                "delta": DELTA,
                "allocation_sigma_upper": allocation_upper["sigma_upper_bracket"],
                "allocation_sigma_lower": allocation_lower["sigma_lower_bracket"],
                "poisson_sigma_upper": poisson_upper["sigma_upper_bracket"],
                "poisson_sigma_lower": poisson_lower["sigma_lower_bracket"],
                "allocation_to_poisson_ratio": (
                    allocation_sigma / poisson_sigma
                ),
                "robust_lower_noise": robust_lower_noise,
                "negative_control_rejected": not (poisson_sigma < poisson_sigma),
            }
        )
    ...
    write_bundle(privacy_rows, utility_rows, passed, runtime)
    summary = {
        "claim_5": "VERIFIED" if passed else "FALSIFIED",
